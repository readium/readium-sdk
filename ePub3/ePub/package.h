...
public:
    //EPUB3_EXPORT
    static
    uint32_t ParseSmilClockValueToMilliseconds(const string& str)
    {
        ...
        if (current == ':')
        {
            current = advanceChar(str, &index); // skip ':' separator
...
private:
    ...
    */
    ...
    void checkDigit(char current, const string& str, size_t* index)
    {
        ...
        {
            throw std::invalid_argument(_Str("The character '", current, "' at position [", index, "] in string \"", str, "\" is not a valid digit !"));
    ...
    {
        throw std::invalid_argument(_Str("The character '", current, "' at position [", index, "] in string \"", str, "\" is not valid !"));
    }
