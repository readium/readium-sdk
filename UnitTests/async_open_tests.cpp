...
{
    std::future<ContainerPtr> future = Container::OpenContainerAsync(EPUB_PATH);
    REQUIRE(future.wait_for(std::chrono::system_clock::duration(0)) != std::future_status::ready);
    ...
    future = Container::OpenContainerAsync(EPUB_PATH, std::launch::deferred);
    REQUIRE(future.wait_for(std::chrono::system_clock::duration(0)) == std::future_status::deferred);
    ...
    future = Container::OpenContainerAsync("bleh.epub", std::launch::async);
    REQUIRE(future.wait_for(std::chrono::system_clock::duration(0)) != std::future_status::ready);
