...
#define MANIFEST_ITEM(manifestItemPtr)    (static_pointer_cast<ePub3::ManifestItem>(jni::Pointer(manifestItemPtr).getPtr()))
...
JNIEXPORT jboolean JNICALL Java_org_readium_sdk_android_ManifestItem_nativeHasProperty(JNIEnv* env, jobject thiz, jlong manifestItemPtr, jint flag) {
    auto manifestItem = MANIFEST_ITEM(manifestItemPtr);
    
    return manifestItem->HasProperty(flag);
}
